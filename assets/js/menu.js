...
async function loadMenu(containerId, jsonFile) {
      ...
      sectionSummary.style.fontSize = '1.5em';
        ...
        itemSummary.style.fontSize = '1.25em';
				...
				if (item.allergens_list?.length) {
					...
					allergens.textContent = `Allergens: ${item.allergens_list.join(' ')}`;
...
loadMenu('grazing-menu-container', 'grazing-menu.json');
loadMenu('elevated-menu-container', 'elevated-menu.json');